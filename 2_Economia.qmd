...
df %>% 
  ...
  fmt_number(dec_mark = ',',
             sep_mark = '.')  %>% 
  tab_source_note(
    source_note = md("Fonte: RAIS. Elaboração: FJP/Diretoria de Estatística e Informações. ")
  ) #define separadores
```
...
df_exportacao <- df_exportacao %>%
  mutate(Municipio = gsub(" - MG", "", rotulos_de_linha)) %>%
  rename(`2023` = soma_de_2023_valor_fob_us,
         ...
         `2015` = soma_de_2015_valor_fob_us)  # Renomeando a coluna

# Classificando os municípios em grupos
df_exportacao <- df_exportacao %>%
  mutate(Grupo = case_when(
    Municipio == "Teófilo Otoni" ~ "Grupo 1",
    Municipio %in% c("Salinas", "Capelinha", "Araçuaí") ~ "Grupo 2",
    Municipio %in% c("Pedra Azul", "Minas Novas", "Medina", "Turmalina", "Itaobim", "Malacacheta") ~ "Grupo 3",
    Municipio %in% c("Itinga", "Virgem da Lapa", "Coronel Murta", "Rubelita") ~ "Grupo 4"
  )) %>%
  arrange(Grupo) %>%
  group_by(Grupo)
...
df_exportacao %>%
  gt() %>%
  tab_header(
    title = "Valor das Exportação por Município"
  ) %>%
  cols_label(
    Municipio = "Município"
  ) %>%
  ...
  tab_style(
    style = cell_text(size = "smaller"),
    locations = cells_body(columns = vars(`2023`))
  ) %>%
  tab_options(
  ...
  tab_source_note(source_note = 'Nota: Valor FOB (US$)')
