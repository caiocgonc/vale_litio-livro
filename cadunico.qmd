---
title: "Utilizando o CadÚnico para os estudos de migração no período recente"
authors: 
- Helena Soares
---
...
## O que os dados do CadÚnico podem revelar sobre características sociais da população em idade ativa presente no cadastro? {#sec-cad-unico}
...
Com base em 2022 como ano de referência 2022 para ambas as bases, apurou o percentual do segmento populacional da faixa etária de 14 a 64 anos, ou seja, em idade ativa para o trabalho cadastrada no CadÚnico em relação ao mesmo segmento etário na população dos municípios com potencial de exploração de lítio no Vale do Jequitinhonha do Censo Demográfico 2022 . No município de Teófilo Otoni, 44,61% de sua população entre 14 e 64 anos encontra-se cadastrada no CadÚnico, seguido de Malacacheta com 53,82%. Os municípios de Coronel Murta e Rubelita possuem o maior percentual da população desse grupo etário inscrito no CadÚnico, com mais de 80% da população em idade ativa para o trabalho cadastrada no Cadastro Único, ou seja, em situação de pobreza monetária.
...
ggplot(df, aes(x = X._POP_Cad_x_municipio_Censo_2022, y = Município, fill = Município)) +
  ...
  labs(title = "Percentual da população do Cadastro Único em idade ativa \n em relação à população do município em idade ativa",
       subtitle = "Municípios no Vale do Jequitinhonha selecionados - 2022",
       x = "População",
       y = "Município") +
  theme_minimal() +
...
ggplot(df_long, aes(x = Ano, y = MUNICIPIO, fill = TaxaAnalfabetismo)) +
  ...
  labs(title = "Taxa de Analfabetismo da população em idade ativa cadastrada no CadÚnico",
       subtitle = "Municípios do Vale do Jequitinhonha selecionados - 2015-2022",
       x = "Ano", y = "Município", fill = "Taxa de Analfabetismo (%)") +
  theme(legend.position = "bottom",
        # plot.title = element_text(hjust = 1, size = 12, face = "bold", margin = margin(b = 0)),
        plot.title = element_text(hjust = 1, size = 13, margin = margin(b = 0)),
        plot.subtitle = element_text(hjust = 0.5, size = 11)) # Ajusta a posição horizontal do título
...
ggplot(df_long, aes(x = Ano, y = TaxaEscolarizacao, group = MUNICIPIO, color = MUNICIPIO)) +
  ...
  labs(title = "Taxa de Escolarização da população em idade ativa para o trabalho (14 a 64 anos) \n inscrita no Cadastro Único (%)",
       subtitle = "Municípios do Vale do Jequitinhonha selecionados - 2012-2022", 
...
ggplot(pop_idade_ativa_long, aes(x = ano, y = Percentual, group = Município, colour = Município)) +
  ...
  geom_point() +
  theme_minimal() +
  labs(title = "Situação de vínculo de trabalho da população \n entre 14 e 64 anos que trabalhou, cadastrada no CadÚnico",
       subtitle = "Municípios do Vale do Jequitinhonha selecionados - 2012-2022",
       y = "%", x = "Ano") +
...
ggplot(df_long, aes(x = MUNICIPIO, y = Proporcao, fill = Genero)) +
  ...
  labs(title = "Proporção da população por sexo",
       subtitle = "Municípios do Vale do Jequitinhonha selecionados - 2022",
       x = "Município",
       y = "Proporção (%)",
       fill = "") +
  theme(legend.position = "bottom",
...
## O que é possível captar de migração com os dados do CadÚnico? {#sec-o-que-é-possível-captar-de-migração-com-os-dados-do-cadúnico}
...
# Criando o gráfico de linhas para a taxa líquida de migração com melhor distinção
 ggplot(saldo_migratorio, aes(x = ano, y = SM, group = Município, colour = Município)) +
  ...
  theme_minimal() +
  labs(title = "Saldo Migratório",
       subtitle = "Municípios do Vale do Jequitinhonha selecionados - 2012-2022",
       y = "Saldo de Migratório",
       x = "Ano") +
  theme(legend.position = "bottom",
...
razao_dep %>% select(-Código) %>% 
  ...
  gt() %>% 
  tab_header(
    title = "Razão de dependência e índice de envelhecimento  - 2010 e 2022",
    subtitle = "Municípios do Vale do Jequitinhonha selecionados - 2012-2022",
  )%>%
