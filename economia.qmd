---
title: "Produção, exportações e o emprego"
editor: visual
authors: 
- Vitor Marinho
- Caio Gonçalves 
...
## Em termos da importância econômica das atividades presentes nos Municípios do Vale do Jequitinhonha selecionados, o que os dados dos PIB revelam? {#sec-qt1-economia}
...
grafico <- ggplot(dados_plot, aes(x = MUNICIPIO, y = Valor, fill = Atividade)) +
  ...
  labs(title = "Valor adicionado total e segundo atividades econômicas",
       subtitle = "Municípios do Vale do Jequitinhonha selecionados - 2021",
       x = "Município",
       y = "R$") +
...
## Qual o panorama atual das exportações dos Municípios do Vale do Jequitinhonha selecionados? {#sec-exportacao}
...
df_exportacao <- df_exportacao %>% 
  ...
  select(rotulos_de_linha,Grupo,`2015`,`2016`,`2017`,`2018`,`2019`,`2020`,`2021`,`2022`,`2023`) %>%
  arrange(Grupo) %>%
  group_by(Grupo) 
...
df_exportacao %>%
  gt() %>%
  tab_header(
    title = "Valor das Exportações FOB (US$)",
    subtitle = "Municípios do Vale do Jequitinhonha selecionados - 2015-2023"
  ) %>%
  cols_label(
  ...
  tab_style(
    style = cell_text(size = "smaller"),
    locations = cells_body(columns = vars(2:11))
  ) %>%
  tab_options(
  ...
  cols_label(
    rotulos_de_linha = "Município") %>% 
  tab_source_note(source_note = 'Nota: Valor FOB (US$)') %>% tab_source_note(source_note = md("Fonte: COMEX-STAT ")) %>% 
  data_color(
    columns = c(2:11),
    colors = my_pal
      )
...
## Qual a importância da indústria extrativa na geração de empregos formais nos Municípios do Vale do Jequitinhonha selecionados? {#sec-emprego-formal}
...
# IDs dos Municípios do Vale do Jequitinhonha selecionados
...
transform(df_vale_litio, Regiao = "Municípios - Lítio"),
...
tx_cres_extrativo$Regiao <- ifelse(tx_cres_extrativo$Regiao=="Vale do Lítio","Municípios - Lítio","Minas Gerais")
...
ggplot(tx_cres_extrativo, aes(x = ano, y = TaxaCrescimento, color = Regiao, group = Regiao)) +
  ...
  labs(
    ...
    subtitle = "Comparação entre o Municípios - Lítio e o estado de Minas Gerais",
    x = "Ano",
