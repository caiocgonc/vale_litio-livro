---
title: "Os fluxos migratórios de Aracuaí com o Censo Demográfico de 2010"
authors: 
- Cláudio Costa
- Helena Soares
---
...
g <- ggplot(dados) +
  ...
  labs(x = "Idade", y = "Percentual (%)", fill = "Sexo") +
  theme_minimal() +
...
```
Fonte: @ibge2023_idade. Censo Demográficos 2022.
...
# Criando o DataFrame
imigracao <- data.frame(Município = nome, UF = uf, Pessoas = pessoas)
```
...
ggplot(imigracao, aes(x = reorder(Município, Pessoas), y = Pessoas, fill = Município)) +
  ...
  labs(title = "Principais municípios que forneceram imigrantes para Araçuaí",
       x = "Municípios", y = "Número de imigrantes") +
  theme_minimal() 
...
As pessoas que moravam em Araçuaí na data de referência do censo de 2010, mas que declararam que moravam em outro município ou país no dia 31/07/2005, têm-se que formavam um contingente de imigrantes que representava um montante de 5,9% (2.122 pessoas) da população total do município.Do total desses imigrantes dos últimos cinco anos, 34,7% (736) pessoas cumpriram alguma etapa migratória entre 2005/2010, antes de chegar a Araçuaí. 
<!-- Os municípios que, em termos de volume, se destacam como destino da etapa migratória intermediária desses migrantes foram: -->
...
emigracao <- data.frame(
  Município = c("Belo Horizonte", "Contagem", "Caraí", "Teófilo Otoni", "Betim", "Jenipapo de Minas", "Vespasiano", "Itinga", "Montes Claros", "Padre Paraíso", "Pitangueiras", "São Paulo", "Brasília", "Araraquara", "Rio de Janeiro", "Guariba", "Pontal", "Elias Fausto", "Américo Brasiliense", "Olímpia"),
...
Em 2010, as pessoas que nasceram e/ou moraram em Araçuaí e residiam em outros municípios formavam um contingente de 5.767 pessoas. Desse total, 3.148 destinaram-se a municípios da própria UF. As outras 2.618 pessoas foram para municípios de outros Estados. A Figura a seguir mostram os principais municípios de destino dos emigrantes de Araçuaí na década de 2000.


```{r}
# Gráfico de barras
ggplot(emigracao, aes(x = reorder(Município, Pessoas), y = Pessoas, fill = Município)) +
  geom_col(show.legend = FALSE) +
  geom_text(aes(label = Pessoas), vjust = -0.02, hjust = 0.97, size = 2.3, color = 'black') +
  coord_flip() +
  labs(title = "Principais municípios de destino dos emigrantes de Araçuaí",
       x = "Municípios", y = "Número de emigrantes") +
  theme_minimal() +
  theme(legend.text = element_text(size = 3.5),  # Ajuste o tamanho do texto da legenda
        legend.key.size = unit(.3, "cm"))       # Ajuste o tamanho do bloco de cor da legenda
...
Em 2010, as pessoas que nasceram e/ou moraram em Araçuaí e residiam em outros municípios formavam um contingente de 5.767 pessoas. Desse total, 3.148 destinaram-se a municípios da própria UF. As outras 2.618 pessoas foram para municípios de outros Estados. 
<!-- A Tabela 4 e a Figura 2 mostram os principais municípios de destino dos emigrantes de Araçuaí na década de 2000. -->
Por outro lado, Araçuaí foi também, nos cinco anos anteriores ao censo de 2010, apenas uma etapa migratória intermediária para 944 pessoas que se locomoveram para outros municípios brasileiros.
...
ggplot(tx_mig_postiva, aes(x = reorder(Nome, Pessoas), y = Pessoas)) +
  ...
  coord_flip() +
  labs(title = "Municípios em que Araçuaí tem Taxa Líquida Migratória Positiva",
       x = " ",
       y = "Pessoas") +
  theme_minimal()
```
...
ggplot(tx_mig_negativa, aes(x = reorder(Nome, Pessoas), y = Pessoas)) +
  ...
  coord_flip() +
  labs(title = "Municípios em que Araçuaí tem Taxa Líquida Migratória Negativa",
       x = " ",
       y = "Pessoas") +
  theme_minimal()
```
