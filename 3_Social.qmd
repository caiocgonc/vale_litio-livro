...
tabela00_social %>%
  ...
  cols_label(
    ...
    is_carente = '30% Mais Carentes'
  ) %>%
  tab_style(
    style = cell_fill(color = "lightcoral"),
    locations = cells_body(vars(is_carente), rows = is_carente)
  ) %>%
  data_color(
  ...
  tab_options(data_row.padding = px(6), heading.align = 'left', column_labels.background.color = 'dodgerblue4', heading.title.font.size = px(20)) %>%
...
FFonte: @fjp2020imrs. Elaboração: FJP/Diretoria de Estatística e Informações.
